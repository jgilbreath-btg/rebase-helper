...
under --new-prefix) are restored the same way as renames.
...
def find_prefixed_changes(
    ...
    return renames, copies, deletions
...
def main() -> int:
    ...
    renames, copies, deletions = find_prefixed_changes(repo, old_prefix, new_prefix)

    for src, dst in copies:
        print(f"[copy, no action] {src} -> {dst} already recognized as a copy; original is already present")

    to_restore = [src for src, _dst in renames] + deletions

    if not to_restore:
        print("No matching renames or deletions found between HEAD~1 and HEAD for the given prefixes.")
        return 0
